...
@section('tab_tittle')
    City Master |  {{$company->company_name}}
@endsection
                                        ...
                                        <div class="row">
                                            <div class="col-md-3">
                                                <button type="button" data-id="{{ $city->id }}"
                                                    class="btn btn-dark btn-sm editCityModel">
                            ...
                            <div class="modal-body">
                                <div class="container">
                                    <div class="row">
                                        <div class="col">
                                            <label class="form-label">{{ __('masters.City') }}</label>
                                            <input type="text" name="city_name_update" id="city_name_update"
                                                class="form-control">
                                            <input type="hidden" id="city_update_id" class="form-control" required>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col">
                                            <div class="mb-3">
                                                <div class="form-label">{{ __('masters.State') }}</div>
                                                <select class="form-select" name="city_state_id"
                                                    id="city_state_id" required>
                                                    <option value="">Choose The State</option>
                                                    @foreach ($stateList as $state)
                                                        <option value="{{ $state->id }}">{{ $state->state_name }}
                                                        </option>
...
@section('scripts')
    <script>
        $(function() {
            ...
            $('body').on('click', '.editCityModel', function() {
                let editId = $(this).attr('data-id')
                let url = "{{ route('admin.city.show', '') }}/" + editId
                axios.get(url)
                    .then(e => {
                        let city_name_update_feild = $('#city_name_update').val(e.data.city_name)
                        let city_state_id_update_feild = $('#city_state_id').val(e.data.state
                            .id)
                        $('#city_update_id').val(e.data.id)
            ...
            $('body').on('click', '#stateUpdateButton', function() {
                let url = "{{ route('admin.city.update', '') }}/" + $('#city_update_id').val()

                axios.patch(url, {
                        'city_name': $('#city_name_update').val(),
